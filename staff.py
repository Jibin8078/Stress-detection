...
def add_result():
    if session.get('lid') is None:
        ...
        if result4:
            qa="select * from result where staff_id='%s'"%(session['sli'])
            ra=select(qa)
            if ra:
                qa="update result set r1='%s',r2='%s',r3='%s',r4='%s' where staff_id='%s'"%(result1,result2,result3,result4,session['sli'])
                update(qa)
            else:
                q="INSERT INTO result(r1,r2,r3,r4,staff_id) VALUES('%s','%s','%s','%s','%s')"%(result1,result2,result3,result4,session['sli'])
                res=insert(q)     
                print(res)
